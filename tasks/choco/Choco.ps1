...
function Execute {
    ...
    # Note we're calling powershell.exe directly, instead of running Invoke-Expression,
    # as suggested by https://learn.microsoft.com/en-us/powershell/scripting/learn/deep-dives/avoid-using-invoke-expression
    # Note that this will run powershell.exe even if the system has pwsh.exe.
    ...
    if ($processExitCode -or $expError) {
        if ($processExitCode -eq 3010) {
            # Expected condition. A reboot is necessary.
        }
        elseif ($expError) {
            throw $expError
        }
